...
For this vignette to be suitable for CRAN submission, it must not
download files to the user's working space.  However, it may 
create a temporary directory and download there, so long as that
directory is deleted at the end.  Such a temporary directory may
...
subs <- paste0(tmpdir, "/", subsRemote)
raws <- paste0(tmpdir, "/", rawsRemote)
for (i in 1:n) {
...
```{r}
# Create a temporary file, so the vignette can build
# on CRAN systems.  In practical work, though, the user
# will almost certainly want to use a non-temporary
# directory.
urlNC <- paste0(
    "https://cproof.uvic.ca/gliderdata/deployments/",
    "dfo-eva035/dfo-eva035-20190718/",
...
fileNC <- paste0(tmpdir, "/", gsub(".*/", "", urlNC))

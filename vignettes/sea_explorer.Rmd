...
```{r} plot(G, which = "p") ``` This verifies the assumption that the early
data were on the shelf.  Note that the device must have been set to return to
the surface when it got to a depth of 1 kilometre at
about 2023-10-27.
...
Q <- quantile(temperature, c(0.01, 0.99)) # avoid outliers
cm <- oce::colormap(temperature, 
                    zlim = Q,
                    col = oceColorsTurbo)
par(mar=c(3, 3, 1, 1))
...
This is starting to look interesting, isn't it?

But let's not get carried away just yet.  Shouldn't we check to see how much we
trust the data?  A temperature-salinity plot can help with that.

```{r}
plot(Gs, which = "TS")
```

Oh, there is a problem here. Notice those low salinities.  This is
a SeaExplorer glider, and we have seen already discussed problems
with spurious values after power-on, so let's look at salinity.

```{r}
plot(Gs, which = "S")
```

Oh yes, those initial salinities are definitely wrong.

At this stage, readers are likely to know how to proceed, but here's a hint:

```{r}
Gs2 <- subset(Gs, salinity > 30)
plot(Gs2, which = "S")
```

This looks better, although it is quite ad-hoc to select this particular
cut-off salinity.  But we are now getting into the territory of advising the
reader how to do science, as opposed to how to use `oceglider`, so it's time to
end this vignette.

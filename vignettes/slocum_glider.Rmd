...
package on Slocum data. It is \emph{very much} a work in progress,
...
A good first step in working with a glider file is to find the names
...
This yields 144 character values, many of which are quite similar.
For example, it might make sense that there only be one variable for
latitude, but we find
...
All but the fourth of the items seems to be related to latitude.  The
one ending `"_qc"` is a quality-control flag, so that can also be
discounted.  Still, there are six possibilities from which to choose.
Inspection of the data file, along with some experiments with all the
choices, suggests, somewhat tentatively, that best choice may be
`"glider_record/m_gps_lat"` (which is used in the default value of
`nameMap`).

Similar study, and a fair bit of testing and discussion, led to
hypotheses for other data items of primary focus, as incorporated in
the `nameMap` default.

Continuing along this line of reasoning, we may extract location data
and make a simple plot with the following, producing Figure 1.
...
A challenge in working through the data, for this file at least, is
that so many items hold NA values, e.g.
...
However, it seems that all the hydrographic variables are finite or NA
together, since
...
![Figure 1. Glider surfaced locations (red dots) with 500m depth
contour (black line). The initial and final locations are at the
northwest end of the dot cloud.](slocum_map.png)

![Figure 2. Dependence of salinity with depth, as sampled along the
path shown in Figure 1.](slocum_tz.png)

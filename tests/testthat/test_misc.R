...
test_that("subset seaexplorer by 'ascending' and descending", {
    ...
    expect_silent(g <- read.glider.seaexplorer.realtime(directory, yo = 3:8, progressBar = FALSE))
...
test_that("subset seaexplorer by pressure", {
    directory <- system.file("extdata/seaexplorer/sub", package = "oceglider")
    expect_silent(g <- read.glider.seaexplorer.realtime(directory, yo = 3, progressBar = FALSE))
    deep <- g[["pressure"]] > 20

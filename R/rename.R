...
renameVocabulary <- function(dictionary = "default.csv", debug = 0) {
    if (is.character(dictionary)) {
        if (!grepl(".csv(.gz){0,1}$", dictionary)) {
            dictionary <- system.file("extdata", paste0("dictionary_", dictionary, ".csv"), package = "oceglider")
        }
    ...
    gliderDebug(debug, "dictionary ", if (is.character(dictionary)) paste0("\"", dictionary, "\" "), "has ", nrow(vocab), " entries\n", sep = "")
    vocab$pattern <- paste0("^", gsub("#", "[0-9]", vocab$original), "$")
    vocab
}

# An internal function, used by renameGlider(); not exported.
# @author Dan Kelley
renameInternal <- function(names, dictionary = "default.csv", debug = 0) {
    debug <- min(3L, max(debug, 0L))
    gliderDebug(debug, "    renameInternal() for glider START\n")
    if (is.character(dictionary) && grepl(".csv(.gz){0,1}$", dictionary)) {
        if (file.exists(dictionary)) {
            gliderDebug(debug, "    using file \"", dictionary, "\"\n", sep = "")
            vocab <- read.csv(dictionary,
                header = FALSE,
                col.names = c("originalName", "oceName", "units", "scale")
            )
            gliderDebug(debug, "  \"", dictionary, "\" has ", nrow(vocab), " entries\n", sep = "")
        } else {
            dictionaryBuiltin <- system.file("extdata", paste0("dictionary_", dictionary), package = "oceglider")
            if (!nchar(dictionaryBuiltin)) {
                stop("file \"", dictionary, "\" does not exist locally or within the package")
            }
            gliderDebug(debug, "    using file \"", dictionaryBuiltin, "\"\n", sep = "")
            vocab <- read.csv(dictionaryBuiltin,
                header = FALSE,
                col.names = c("originalName", "oceName", "units", "scale")
            )
        }
        ...
        vocab <- dictionary
        names(vocab) <- c("originalName", "oceName", "units", "scale")
        gliderDebug(debug, "data-frame dictionary has ", nrow(vocab), " entries\n")
    }
    ...
    for (i in seq_along(names)) {
        ...
        wlen <- length(w)
        if (wlen == 1) {
            gliderDebug(debug, "  * \"", name, "\" is an exact match for vocabulary at index ", w, "\n")
        }
        if (length(w) == 0L) {
            # gliderDebug(debug, "  no exact match, so searching patterns for a match\n")
            w <- which(sapply(vocab$pattern, \(pattern) grepl(pattern, name)))
            # gliderDebug(debug, vectorShow(w))
            if (length(w) > 1L) {
                warning("multiple name match on \"", name, "\"; using first", sep = "")
                w <- w[1]
            }
            if (length(w) == 1L) {
                gliderDebug(debug, "  * \"", name, "\" is a pattern match to vocabulary at index ", w, "\n")
            }
        }
        if (length(w) == 0L) {
            gliderDebug(debug, "  * \"", name, "\" is not in vocabulary; so not renamed\n", sep = "")
            rval <- rbind(rval, c(originalName = name, oceName = name, unit = "", scale = ""))
        } else {
            if (length(w) == 1L) {
                gliderDebug(debug, "  \"", name, "\" matches vocabulary at index ", w, "\n", sep = "")
            } else {
                gliderDebug(debug, "  \"", name, "\" matches vocabulary at indices ",
            ...
            gliderDebug(debug, vectorShow(vocab[w, ]))
            newrow <- c(
                originalName = name,
    ...
    gliderDebug(debug, "handling duplicated names\n")
...
#'
#' ```
#' system.file("extdata", "dictionary_ioos.csv", package = "oceglider")
#' ```
#'
...
#'
#' @author Dan Kelley
renameGlider <- function(x, dictionary = "default", debug = 0) {
    gliderDebug(debug, "rename(..., dictionary=",
    ...
    if (!inherits(x, "glider")) stop("x is not an glider-class object")
    # if (!is.character(dictionary)) {
    ...
    gliderDebug(debug, "finding vocabulary translations\n")
    R <- renameInternal(originalNames, dictionary = dictionary, debug = debug - 1)
    gliderDebug(debug, "setting up @metadata$dataNamesOriginal\n")
    ...
    # message("DAN 3")
    gliderDebug(debug, "setting up @metadata$units\n")
    if (0L == length(x@metadata$units)) {
        ...
        gliderDebug(debug, "renaming units one by one\n")
        # message("DAN 1")
    ...
    gliderDebug(debug, "move unit-length items in data slot to metadata slot\n")
    ...
    gliderDebug(debug, "handling flags\n")
    dataNames <- names(rval@data)
    if (!is.null(rval@metadata$flags)) {
        oldFlagNames <- names(rval@metadata$flags)
        gliderDebug(debug, "  ", vectorShow(oldFlagNames, n = 100))
        ...
        gliderDebug(debug, "  ", vectorShow(newFlagNames, n = 100))
        # print(R)
